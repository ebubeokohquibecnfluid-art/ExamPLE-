...
export const db = {
  ...
  run: async (query: string, params: any[] = []) => {
    ...
    else if (q.includes("update users set credits = 0, expiry_date = null where uid = ?")) {
      await firestore.collection('users').doc(params[0]).update({
        credits: 0,
        expiry_date: null
      });
    }
    else if (q.includes("update users set credits = credits +")) {
      await firestore.collection('users').doc(params[1]).update({
        credits: FieldValue.increment(params[0]),
        expiry_date: params.length > 2 ? params[2] : null
      });
    }
    return { changes: 1 };
  },
