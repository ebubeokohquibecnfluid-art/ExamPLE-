...
function MainApp({ user, profile, onLogin, onLogout, refreshProfile, showToast, showSettings, setShowSettings }: { user: any | null, profile: UserProfile | null, onLogin: () => void, onLogout: () => void, refreshProfile: () => void, showToast: (msg: string, type?: any) => void, showSettings: boolean, setShowSettings: (show: boolean) => void }) {
  ...
  useEffect(() => {
    if (school_slug) {
      const fetchSchool = async () => {
        try {
          const res = await fetch(`/api/schools/by-slug/${school_slug}`);
          if (res.ok) {
            const data = await res.json();
    ...
    try {
      ...
      if (data.message.includes("Welcome to ExamPLE")) {
        ...
        if (schoolMatch && schoolMatch[1]) {
          const slugRes = await fetch(`/api/schools/by-slug/${schoolCodeInput.toLowerCase()}`);
          if (slugRes.ok) {
  ...
  const handleSchoolRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/register-school", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
...
export default function App() {
  ...
  return (
    <ErrorBoundary>
      <BrowserRouter>
        <Routes>
          <Route path="/" element={<MainApp user={user} profile={profile} onLogin={() => setShowLoginModal(true)} onLogout={handleLogout} refreshProfile={() => user && fetchProfile(user.uid)} showToast={showToast} showSettings={showSettings} setShowSettings={setShowSettings} />} />
          <Route path="/admin" element={<AdminDashboard showToast={showToast} />} />
          <Route path="/:school_slug" element={<MainApp user={user} profile={profile} onLogin={() => setShowLoginModal(true)} onLogout={handleLogout} refreshProfile={() => user && fetchProfile(user.uid)} showToast={showToast} showSettings={showSettings} setShowSettings={setShowSettings} />} />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <button onClick={() => { setShowLoginModal(false); setLoginStep('choice'); }} className="p-2 hover:bg-slate-100 rounded-full transition-all">
                  <X className="w-6 h-6 text-slate-400" />
                </button>
              </div>
