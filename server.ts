...
async function startServer() {
  ...
  app.post("/api/transcribe", async (req, res) => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: [{ inlineData: { data: audioBase64.split(',')[1] || audioBase64, mimeType: "audio/webm" } }, { text: "Transcribe this audio." }],
      });
  ...
  app.post("/ask-question", async (req, res) => {
    ...
    try {
      ...
      if (currentCreditsVal < 1) return res.status(403).json({ error: "Insufficient credits." });
      ...
      const stream = await model.generateContentStream(promptParts);
...
startServer().catch(err => { console.error("Startup Crash:", err); process.exit(1); });
